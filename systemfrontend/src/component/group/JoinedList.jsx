...
const JoinedList = (props) => {
    ...
    const onSubmit = (e) => {
        ...
        axios.post(`http://localhost:8080/api/group/removeFromGroup?name=${selectedRow.name}`, getList(selected.current), config)
            .then(res => {
                console.log(res)
    ...
    return (
        <div>
            <div className="groupBox">
                <Datatable fcolumns={membersSimplifycolumns} fdata={data} routlink="" title="Members" formCode="" isButton={false} isUserList={false} isGroupList={false} isCheckBox={true} isMemberList={true} selected={selected} />
            </div>
            <div className="buttonClass">
                <button onClick={(e) => { onSubmit(e) }}>Remove Members</button>
            </div>
        </div>
